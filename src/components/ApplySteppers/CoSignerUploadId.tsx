...
const CoSignerUploadId = ({ formData, setFormData, setalcStep, handleSubmit }) => {
    const [isSkipId, setIsSkipId] = useState(0);
    const [hasError, setHasError] = useState(0);
    const handleFileChange = (event) => {
        setHasError(0);
        const file = event.target.files[0];
        setFormData({ ...formData, co_signer_driving_license_img: file });
    };

    const handleNext = () => {
        if (formData.co_signer_driving_license_img === "") {
    ...
    return (
        <div className='stepper-content'>
            ...
            <div className='upload-img-btn'>
                ...
                {formData.co_signer_driving_license_img ? <p>{formData.co_signer_driving_license_img.name}</p> : <p>No file chosen</p>}
            </div>
            ...
            <div className="error-messages">
                {hasError === 1 ?
                    <ul>
                        {formData.co_signer_driving_license_img === "" ? <li>Upload valid id</li> : ""}
                    </ul> : ""
                }
            </div>
            {isSkipId ? 
...
export default CoSignerUploadId;
